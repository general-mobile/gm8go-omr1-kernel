...
#if CHIPONE_POWER_SOURCE_CUST_EN
/*****************************************************************************
* Power Control
*****************************************************************************/
bool power_disabled = false;
int chipone_power_init(void)
{
    int ret;
    /*set TP volt*/
...
int chipone_power_suspend(void)
{
    int ret;
    if(!power_disabled){
        ret = regulator_disable(tpd->reg);
        if (ret != 0)
            printk("[POWER]Failed to disable regulator when suspend ret=%d!", ret);
        else
            power_disabled = true;
    }
...
int chipone_power_resume(void)
{
    ...
    if(power_disabled){
        ret = regulator_enable(tpd->reg);
        if (ret != 0)
            printk("[POWER]Failed to enable regulator when resume,ret=%d!", ret);
        else
