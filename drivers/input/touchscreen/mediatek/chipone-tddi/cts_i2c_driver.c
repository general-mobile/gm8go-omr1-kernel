...
extern u8 tpd_proximity_suspend;
...
/*******************************/
	...
	{
                ret = cts_plat_release_all_touch(cts_data->cts_dev.pdata);
		printk("suspend cts_release_all_touch!!!");
                if (ret) {
                        cts_err("suspend Release all touch failed %d", ret);
                        //return ret;
                }
                //cts_dev->rtdata.suspended = true;
		printk("Proximity suspend enter!!!");
		return 0;
	}
#endif
/*******************************/
...
if(tpd_proximity_flag)
	{
                ret = cts_plat_release_all_touch(cts_data->cts_dev.pdata);
		printk("resume cts_release_all_touch!!!");
                if (ret) {
                        cts_err("resume Release all touch failed %d", ret);
                        //return ret;
                }
