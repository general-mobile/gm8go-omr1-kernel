/* Himax Android Driver Sample Code for HX83110 chipset
*
...
int himax_chip_self_test(struct i2c_client *client)
{
    ...
    tmp_addr[1] = 0x7F;
    tmp_addr[0] = 0x24;
    himax_register_read(client, tmp_addr, 12, tmp_data, false);
...
void himax_interface_on(struct i2c_client *client)
{
    ...
    if ( i2c_himax_write(client, 0x13,tmp_data, 1, DEFAULT_RETRY_CNT) < 0)   // to knock I2C
    {
        E("%s: i2c access fail!\n", __func__);
        return;
    }
...
int fts_ctpm_fw_upgrade_with_sys_fs_64k(struct i2c_client *client, unsigned char *fw, int len, bool change_iref) //Alice - Un
{
    ...
    if (himax_XX42_check_CRC(client,tmp_data, FW_SIZE_64k) == 0)
    {
        burnFW_success = 1;
    }
    else
...
void himax_touch_information(struct i2c_client *client)
{
    ...
    if(IC_TYPE == HX_83110A_SERIES_PWON)
    {
        cmd[3] = 0x08;
        cmd[2] = 0x00;
        cmd[1] = 0x00;
...
int himax_read_ic_trigger_type(struct i2c_client *client)
{
    ...
    result = i2c_himax_write(client, 0x00,tmp_data, 4, DEFAULT_RETRY_CNT);
    if (result < 0)
    {
        E("%s: i2c access fail!\n", __func__);
        return result;
    ...
    tmp_data[0] = 0x00;
    result = i2c_himax_write(client, 0x0C,tmp_data, 1, DEFAULT_RETRY_CNT);
    if (result < 0)
    {
        E("%s: i2c access fail!\n", __func__);
        return result;
    }
    result = i2c_himax_read(client, 0x08,read_data, 1, DEFAULT_RETRY_CNT);
    if (result < 0)
    {
        E("%s: i2c access fail!\n", __func__);
        return result;
    }
    return read_data[0] & 0x01;
}
...
void himax_get_DSRAM_data(struct i2c_client *client, uint8_t *info_data)
{
    ...
    //1. Read number of MKey R100070E8H
    tmp_addr[3] = 0x10;
    tmp_addr[2] = 0x00;
    tmp_addr[1] = 0x70;
    tmp_addr[0] = 0xE8;
    himax_register_read(client, tmp_data, 4, tmp_addr, false);

    m_key_num = tmp_data[0] & 0x03;

    total_size += m_key_num;

    //2. Start DSRAM Rawdata
    tmp_addr[3] = 0x10;
    tmp_addr[2] = 0x00;
    tmp_addr[1] = 0x00;
    tmp_addr[0] = 0x00;
    ...
    for (i = 0; i < (total_read_times); i++)
    {
        ...
        {
            himax_register_read(client, tmp_addr, total_size % max_i2c_size, &temp_info_data[i*max_i2c_size], false);
        }
    ...
    total_size = (x_num * y_num + x_num + y_num) * 2 + m_key_num + 4;
...
bool himax_calculateChecksum(struct i2c_client *client, bool change_iref)
{
    ...
    CRC_result = himax_XX42_check_CRC(client,tmp_data, FW_SIZE_64k);
