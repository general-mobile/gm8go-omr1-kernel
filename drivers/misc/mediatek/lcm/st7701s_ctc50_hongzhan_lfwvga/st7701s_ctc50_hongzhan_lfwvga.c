...
#define LCM_PHYSICAL_WIDTH                                  (56160)
#define LCM_PHYSICAL_HEIGHT                                 (112320)
...
static void lcm_get_params(LCM_PARAMS *params)
{
  ...
  params->dsi.horizontal_backporch				= 164;//46;
  params->dsi.horizontal_frontporch				= 160;//46;
  params->dsi.horizontal_active_pixel				= FRAME_WIDTH;

  params->dsi.ssc_disable= 1;
  params->dsi.PLL_CLOCK = 290; //dpi clock customization: should config 
...
#if defined(BUILD_LK)
	printf("st7701_ctc50_hongzhan_fwvga  %s,line=%d, device_id = 0x%x, buffer[0]=0x%08x,buffer[1]=0x%08x\n",__func__,__LINE__, device_id, buffer[0],buffer[1]);
#else
	printk("st7701_ctc50_hongzhan_fwvga  %s,line=%d, device_id = 0x%x, buffer[0]=0x%08x,buffer[1]=0x%08x\n",__func__,__LINE__, device_id, buffer[0],buffer[1]);
#endif
...
LCM_DRIVER st7701s_ctc50_hongzhan_lfwvga_lcm_drv =
{
  .name		  = "st7701s_ctc50_hongzhan_lfwvga",
