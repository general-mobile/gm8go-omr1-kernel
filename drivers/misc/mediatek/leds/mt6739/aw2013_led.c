...
#define Rise_time   0x00   //LED rise time,0x00=0.13s,0x01=0.26s,0x02=0.52s,0x03=1.04s,0x04=2.08s,0x05=4.16s,0x06=8.32s,0x07=16.64s
#define Hold_time   0x01   //LED max light time light 0x00=0.13s,0x01=0.26s,0x02=0.52s,0x03=1.04s,0x04=2.08s,0x05=4.16s
#define Fall_time     0x00   //LED fall time,0x00=0.13s,0x01=0.26s,0x02=0.52s,0x03=1.04s,0x04=2.08s,0x05=4.16s,0x06=8.32s,0x07=16.64s
...
void aw2013_breath_led0(int led0)  //led on=0x01   ledoff=0x00
{  
    printk("%s\n",__func__);
	I2C_write_reg(0x01, 0x01);				// enable LED 		
	...
	I2C_write_reg(0x30, led0);		//led on=0x01 ledoff=0x00	
}
void aw2013_breath_led1(int led1)  //led on=0x01   ledoff=0x00
{  
    printk("%s\n",__func__);	
	I2C_write_reg(0x01, 0x01);				// enable LED 		
	...
	I2C_write_reg(0x30, led1<<1);		//led on=0x01 ledoff=0x00	
}
...
void aw2013_led_led0(int led0)
{
	...
	reg_value = I2C_read_reg(0x30);
        reg_value &= ~(1<<0);
        reg_value |= (led0<<0);
	I2C_write_reg(0x30, reg_value);		//led on=0x01 ledoff=0x00	
}
void aw2013_led_led1(int led1)
{
    unsigned char reg_value;
    printk("%s [%d]\n",__func__,led1);
	I2C_write_reg(0x01, 0x01);				// enable LED 		
	...
	I2C_write_reg(0x35, 0xff);				// LED0 level,
