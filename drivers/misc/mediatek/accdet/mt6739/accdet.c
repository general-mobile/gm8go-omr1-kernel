...
static int Accdet_PMIC_IMM_GetOneChannelValue(int key_check)
{
	...
	int sum=0;
        ...
        for(i=0;i<5;i++)
        {
           vol_val[i] = pmic_get_auxadc_value(AUXADC_LIST_ACCDET);
           sum += vol_val[i];
           //printk("vol_val[%d] = %d ,sum = %d\n",i,vol_val[i],sum);
        }
        vol_val[1]=sum/5;
	if (key_check) {
		if (vol_val[1] > g_accdet_auxadc_offset)
			vol_val[1] -= g_accdet_auxadc_offset;
		else
			vol_val[1] = 0;
		ACCDET_DEBUG("[accdet] adc_offset=%d mv,MIC_Vol=%d mv\n", g_accdet_auxadc_offset, vol_val[1]);
	}
	return vol_val[1];
}
