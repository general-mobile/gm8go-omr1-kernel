...
#include "sp5509mipi_Sensor.h"
//extern int main_sub_flag;
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0x58455515,		//checksum value for Camera Auto Test
  ...
  .custom5 = {
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,//sensor output first pixel color Gb B  // Gr
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    kal_uint8 i = 0;
    kal_uint8 retry = 2;
	LOG_INF(" H52028_SP5509 get_imgsensor_id() sensor_id = %d \n",*sensor_id); 
/*		if(main_sub_flag != 0)
	{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			printk("H52028_SP5509 i2c write id: 0x%x,sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
            if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("H52028_SP5509 i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
                return ERROR_NONE;
            }
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		// 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK
		 write_cmos_sensor(0x0a04, 0x0141);
		 write_cmos_sensor(0x0200, 0x0001);
			write_cmos_sensor(0x0201, 0x0008);
		 write_cmos_sensor(0x0206, 0x000a);
		 write_cmos_sensor(0x0208, 0x0a0a);
		 write_cmos_sensor(0x020a, 0x000a);
		 write_cmos_sensor(0x020c, 0x0a0a);
