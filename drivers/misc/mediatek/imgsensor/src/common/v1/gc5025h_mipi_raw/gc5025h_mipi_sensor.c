...
static kal_bool GC5025HDuringTestPattern = KAL_FALSE;

static kal_uint32 Dgain_ratio = 1;

static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC5025HMIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static void gc5025h_gcore_read_otp_info(void)
{
	...
	case 0x01:	
		...
		{
			LOG_INF("GC5025H_OTP_DD : DD check sum correct! checksum = 0x%x\n", ddchecksum);
		}
		else
		{
		...
		{
			LOG_INF("GC5025H_OTP_DD:index = %d, data[0] = %x , data[1] = %x , data[2] = %x ,data[3] = %x \n",\
...
//INFO&WB
	for(index=0;index<2;index++)
	{
		...
		case 0x01:
			...
			{
				LOG_INF("GC5025H_OTP_INFO Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x02:
		case 0x03:	
		...
		case 0x01:
			...
			{
				LOG_INF("GC5025H_OTP_WB Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x02:
		case 0x03:	
		...
		case 0x01:
			...
			{
				LOG_INF("GC5025H_OTP_GOLDEN Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x02:
		case 0x03:	
...
static void gc5025h_gcore_update_chipversion(void)
{
	...
	for(i=0;i<gc5025h_otp_info.reg_num;i++) 
	{
		...
		LOG_INF("GC5025H_OTP_UPDATE_CHIP_VERSION:{0x%x,0x%x}!!\n",gc5025h_otp_info.reg_addr[i],gc5025h_otp_info.reg_value[i]);		
	}
